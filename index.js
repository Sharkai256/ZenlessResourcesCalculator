"use strict";
// TODO: css
// TODO: wallpaper changes depending on the chosen character. default - zenless wallpaper
//? check other ways to add js before DOMLoaded
window.addEventListener('DOMContentLoaded', () => {
    ...
    const characters = [
        ...
        },
        {
            ...
            type: 'Support',
            attribute: 'Fire',
            core: ['Ferocious Grip', 'Steel Malice']
        },
        {
            name: 'Piper',
            type: 'Anomaly',
            attribute: 'Physical',
            core: ['Finale Dance Shoes', 'Crimson Awe']
        }
    ];
    const addDOMElement = (elememt, HTML, destination, insertFirst, insertBefore, target) => {
        elememt.innerHTML += HTML;
        if (!insertBefore || !target) {
            if (!insertFirst)
                destination?.append(elememt);
            else
                destination?.insertBefore(elememt, destination.firstChild);
        }
        else
            destination?.insertBefore(elememt, target);
        return elememt;
    };
    const calculateResources = (resourceInputs, coreInputs, farmRow) => {
    ...
    const activateTables = () => {
        if (!characterTableBody || !characterTableHead)
            throw new Error('Character table does not exist');
        if (!farmTableHead || !farmTableBody)
            throw new Error('Farm table does not exist');
        ...
        if (!characterSelect)
            throw new Error('Character select does not exist');
        characterSelect.addEventListener('change', () => {
            ...
            farmTableHead.children[0].innerHTML = `<th>${characters[choice].core[0]}</th>`
                ...
                + '<th>WEngine Promo</th>';
            farmRow.innerHTML = '<td>9</td><td>60</td><td>250</td><td>5</td><td>30</td><td>30</td>';
            // resource inputs are in separate data tables and cannot be manipulated as standart array elements
            ...
            if (!levelSelect)
                throw new Error('Level selects does not exist');
            if (!WEngineSelect)
                throw new Error('WEngigne selects does not exist');
            ...
            levelSelect.addEventListener('change', () => {
                let maxFarmLevel = 30;
                if (+levelSelect.value == 50)
                    maxFarmLevel -= 10;
                if (+levelSelect.value == 60)
                    maxFarmLevel -= 30;
            ...
            WEngineSelect.addEventListener('change', () => {
                let maxFarmLevel = 30;
                if (+WEngineSelect.value == 50)
                    maxFarmLevel -= 10;
                if (+WEngineSelect.value == 60)
                    maxFarmLevel -= 30;
	...
	<td>-</td>
	<td>-</td>`;
    addDOMElement(document.createElement('div'), guideHTML, body, true);
    addDOMElement(document.createElement('tr'), charHeaderHTML, characterTableHead);
    addDOMElement(document.createElement('tr'), charBodyHTML, characterTableBody);
    addDOMElement(document.createElement('tr'), ``, farmTableHead);
    addDOMElement(document.createElement('tr'), ``, farmTableBody);
    activateTables();
});
